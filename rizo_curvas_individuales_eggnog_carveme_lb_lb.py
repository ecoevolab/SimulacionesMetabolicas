...
import numpy as np
# -----------------------------------
csv_files = glob.glob('./04_resultados/rizo/biomasas/*_eggnog_carveme_lb.csv')
colores_bac = {
...
for file in csv_files:
    ...
    model_id = file_name.replace('_eggnog_carveme_lb.csv', '')
    ...
    try:
        ...
        plt.xlabel('''Tiempo (Ciclos de Simulación)
        Fig. 1. Genomas anotados con eggnoteg, modelo reconstruido con CarveMe y Gapfilling con medio LB.''')
        plt.ylabel('Biomasa [ln(g)]')
        plt.grid(True, linestyle='--', alpha=0.6)
        output_folder = '04_resultados/rizo/graficas'
        output_path = os.path.join(output_folder, f"{model_id}__eggnog_carveme_lb_lb.png")
