...
output_path = '/mnt/data/sur/users/mmontante/04_resultados/'
if not os.path.exists(output_path):
    os.makedirs(output_path)
    
folder_temp = '/mnt/data/sur/users/mmontante/foldert'
...
model_path = '/mnt/data/sur/users/mmontante/02_data/rizo/carveme/ST00060_prokka_carveme_lb.xml'
cobra_model = cobra.io.read_sbml_model(model_path)
ST60 = c.model(cobra_model)
ST60.id = 'ST60'
...
comp_params.set_param('maxCycles', 50)
...
# Serie de Tiempo de Metabolitos
df_metabolites = comp_assay.get_metabolite_time_series()
df_metabolites.to_csv(os.path.join(output_path, 'serie_tiempo_metabolitos_60.csv'), index=False)

# Biomasa Total
df_biomasa = comp_assay.total_biomass
# Añadimos columna de tiempo real (cycle * timeStep)
df_biomasa['time'] = df_biomasa['cycle'] * 0.1
df_biomasa.to_csv(os.path.join(output_path, 'crecimiento_biomasa_60.csv'), index=False)

print(f"Simulación terminada. Resultados en: {output_path}")
